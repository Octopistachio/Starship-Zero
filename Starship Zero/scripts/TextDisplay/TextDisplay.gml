...
draw_text(xpos,ypos,string_hash_to_newline(chars[0]));//draw the first character,PS:you can remove this and put it in the next for loop but u need to put chars_drawn to 0 in the create event
cur_width+=string_width(string_hash_to_newline(chars[0]));//add the first character's width to the total width
    ...
    {   
        if (cur_width>=room_width - boxMargin - boxOutlineWidth - textMargin*8) //If the text in the box is longer than the box
        {
            ...
            if(chars[j]!=" "){ //If the character going to the new line is not a space
                draw_text(xpos+cur_width,ypos+line,string_hash_to_newline("-")); //Put a hyphen in
                line+=string_height(string_hash_to_newline(chars[j-1]));
                cur_width=0;
                draw_text(xpos+cur_width,ypos+line,string_hash_to_newline(chars[j])); 
                cur_width+=string_width(string_hash_to_newline(chars[j]));
            }
